              ...
              <AnimatePresence>
                {blocks.map((block) => (
                  <motion.div
                    ...
                    className="group relative border-2 border-transparent hover:border-primary/50 transition-colors"
                  >
                    {/* Controles do Bloco (Hover) */}
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-20 flex gap-2">
                      <button className="p-2 rounded-md bg-panel border border-border text-textSecondary hover:text-white shadow-lg cursor-grab active:cursor-grabbing">
                        <GripVertical className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleRemoveBlock(block.id)} className="p-2 rounded-md bg-panel border border-border text-error hover:bg-error/10 shadow-lg">
                    ...
                    {block.type === 'features' && (
                      <div className="py-20 px-6 md:px-12 bg-panel">
                        ...
                        <div className="grid md:grid-cols-3 gap-6">
                          {[block.content.f1, block.content.f2, block.content.f3].map((f, i) => (
